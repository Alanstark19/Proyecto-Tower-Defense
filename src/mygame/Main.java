...
public class Main extends SimpleApplication {
    ...
    private Geometry myBox(String name) {
        ...
        int numero = (int) (Math.random() * (-65 +35) );//int numero = (int) (Math.random() * 20 + 1);
 
        //creamos los enemigos;
        //Creamos una esfera para nuestros enemigos llamados covichos
        Sphere covicho = new Sphere(20, 20, 1);
        ...
        //establecemos el material a nuestra geometria
        covicho_mat.setTexture("ColorMap", assetManager.loadTexture("Textures/covicho.jpg"));
    ...
    /**
     * Utilizamos el listener analogico ya que la accion de eliminar será una accion continua.
     */
    private final AnalogListener analogListener = new AnalogListener() {
        @Override
        public void onAnalog(String name, float intensity, float tpf) {
            // se comprueba que el trigger indentificado corresponda a la acción deseada
            if (name.contains(MAPPING_SHOT)) {
                ...
                if (results.size() >= 0) {
                    ...
                    //String tar = results.getFarthestCollision().getGeometry().getName();
                    Geometry target2 = results.getClosestCollision().getGeometry();
                   // String tar2 = results.getFarthestCollision().getGeometry().getName();
    ...
    public void simpleUpdate(float tpf) {
       //crearEnemigosTimer();
        ...
        for (Spatial misSpatial1 : misSpatial) {
            ...
            misSpatial1.move(-FastMath.HALF_PI / 20, 0, 0);
